...
from app.models.medical_record_model import medical_record_model
...
def get_medical_record_by_id(db: Session, record_id: int):
	return db.query(medical_record_model.MedicalRecord).filter(medical_record_model.MedicalRecordCreate.id == record_id).first()
	
def get_records_by_patient(db: Session, patient_id: int):
	return db.query(medical_record_model.MedicalRecord).filter(medical_record_model.patient_id == patient_id).all()
