...
from app.models.doctor_model import doctor_model
from app.schemas.doctor_schema import doctor_schema

def create_doctor(db: Session, doctor: doctor_schema.DoctorCreate):
	db_doctor = doctor_model.Doctor(**doctor.doct())
	db.add(doctor)
	db.commit()
...
def update_doctor(db: Session, doctor_id: int, doctor:doctor_schema.DoctorUpdate):
...
def delete_doctor(db:session, doct_id: int)
	db_doctor = get_doctor_by_id(db, doctor_id)
	if db_doctor:
		db.delete(db_doctor)
		db.commit()
